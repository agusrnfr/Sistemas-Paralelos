...
void *intersec_vector(void *ptr)
{
    ...
    double *intersec_local = (double *)malloc(sizeof(double) * N);
    ...
    for (int i = inicio; i < fin; i++)
    {
        for (int j = 0; j < N; j++)
        {
            if (A[i] == B[j])
            {
                intersec_local[local_k++] = A[i];
                break;
            }
        }
    }
    
    pthread_mutex_lock(&mutexIntersec);
    for (int i = 0; i < local_k; i++)
    {
        INTERSEC[k++] = intersec_local[i];
    }
    pthread_mutex_unlock(&mutexIntersec);
