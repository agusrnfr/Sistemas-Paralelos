...
{
	int i, j, k, numProcs, rank, stripSize, check = 1, provided;
	...
	if ((argc != 4) || (n <= 0) || (bs <= 0) || ((n % bs) != 0) || (hilos <= 0))
	{
		printf("\nParámetros inválidos. Debe utilizar %s N BS hilos (N debe ser múltiplo de BS)\n", argv[0]);
		exit(1);
	}
...
#pragma omp parallel private(i, j, k, offsetI, offsetJ)
	{
#pragma omp for schedule(static) nowait
		for (i = 0; i < stripSize; i += bs)
		{
			...
			for (j = 0; j < n; j += bs)
			{
				...
				for (k = 0; k < n; k += bs)
				{
					blkmul(&a[offsetI + k], &b[offsetJ + k], &r1[offsetI + j], n, bs);
				}
			}
		}
#pragma omp for schedule(static) nowait
		for (i = 0; i < stripSize; i += bs)
		{
			...
			for (j = 0; j < n; j += bs)
			{
				...
				for (k = 0; k < n; k += bs)
				{
					blkmul(&c[offsetI + k], &d[offsetJ + k], &r2[offsetI + j], n, bs);
				}
			}
		}
#pragma omp for schedule(static) nowait
		for (i = 0; i < stripSize; i += bs)
		{
			...
			for (j = 0; j < n; j += bs)
			{
				...
				for (k = 0; k < n; k += bs)
				{
					blkmul(&e[offsetI + k], &f[offsetJ + k], &r3[offsetI + j], n, bs);
				}
			}
		}
	}
...
#pragma omp parallel for num_threads(hilos) private(i, j)
